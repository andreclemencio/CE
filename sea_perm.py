...
def muta_scramble(indiv, prob_muta):
	...
	if value < prob_muta:
		ind = sample(range(len(cromo)),2)
		ind.sort()
		i1, i2 = ind
		scramble = cromo[i1:i2]
		shuffle(scramble)
		cromo = cromo[:i1] + scramble + cromo[i2:]

	return cromo


def muta_insertion(indiv, prob_muta):
    cromo = indiv[:]
    value = random()
    if value < prob_muta:
